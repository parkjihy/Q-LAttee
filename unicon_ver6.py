...
model = AttnLSTM(input_size, hidden_units)
...
for epoch in range(num_epochs):
    ...
    for features, targets in train_dataloader:
     ...
     # Print average loss and RMSE for each epoch
    avg_loss=total_loss/len(train_dataloader)
    rmse=(total_squared_error / len(train_dataset))**0.5
    print(f"Epoch {epoch+1}/{num_epochs}, Average Loss: {avg_loss:.4f}, RMSE: {rmse:.4f}")
...
with torch.no_grad():
    ...
    for features, targets in test_dataloader:
     ...
     # Print average loss and RMSE for the test set
    avg_loss=total_loss/len(test_dataloader)
    rmse=(total_squared_error / len(test_dataset))**0.5
    print(f"Test Average Loss: {avg_loss:.4f}, Test RMSE: {rmse:.4f}")
...
# Calculate MASE
MASE = calculate_MASE(actual_values, predictions)
...
# Set a Seaborn style with a white background
sns.set(style="whitegrid")

# Create a custom color map for the background gradient
colors = [(0.9, 0.9, 0.9), (1.0, 1.0, 1.0), (0.9, 0.9, 0.9)]  # Light to dark gray
cmap = LinearSegmentedColormap.from_list('custom', colors, N=256)

# Create a figure with a white background
fig, ax = plt.subplots(figsize=(10, 6))
fig.patch.set_facecolor('#ffffff')  # White background color
ax.set_facecolor('#ffffff')  # White background color

# Plot the actual values without points
plt.plot(actual_values, label='Actual Values', color='blue', alpha=0.8, linewidth=2)

# Plot the predicted values without points
plt.plot(predictions, label='Predicted Values', color='orange', alpha=0.8, linewidth=2)

# Add labels and title with padding
plt.xlabel('Sample Index', fontsize=14, color='black')
plt.ylabel('Energy Consumption (kWh)', fontsize=14, color='black')
title = plt.title('Total Energy Consumption', fontsize=18, color='black', pad=20)

# Set the title background color to match the plot background
title.set_bbox({'facecolor': '#ffffff', 'edgecolor': '#ffffff'})
...
plt.grid(True, linestyle='--', alpha=0.4, color='#dddddd')  # Styled grid
plt.xticks(fontsize=10, color='black')
plt.yticks(fontsize=10, color='black')

# Save the plot to a file (e.g., a PNG file)
plt.savefig('/home/unicorn/unicon/lstm.png', bbox_inches='tight', facecolor=fig.get_facecolor(), transparent=True)
...
# Define custom dataset for loading and preprocessing the data
class EnergyDataset(Dataset):
    def __init__(self, csv_file):
        self.data = pd.read_csv(csv_file)
        self.features = self.data[features].values.astype(float)
        self.targets = self.data['total'].values.astype(float)
...
# Define a simple transformer model for regression task
class RegressionTransformer(nn.Module):
    def __init__(self, input_dim):
        super(RegressionTransformer, self).__init__()
        self.transformer_encoder = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=output_dim, nhead=1), num_layers=1)
        self.linear = nn.Linear(input_dim , 5) # Output layer
...
model = RegressionTransformer(input_size)
...
# Save the plot to a file (e.g., a PNG file)
plt.savefig('/home/unicorn/unicon/transformer.png', bbox_inches='tight')
